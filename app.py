...
# Platform defaults for fuel fixation (used when equipment-level value is unset/0)
# Default to 100 if .env or env var is not present
DEFAULT_FUEL_FIXATION = float(os.environ.get('DEFAULT_FUEL_FIXATION', '100'))
DEFAULT_FUEL_UNIT = os.environ.get('DEFAULT_FUEL_UNIT', 'per_rental')
...
class Equipment(db.Model):
    ...
    fuel_fixation = db.Column(db.Float, default=0.0)  # Fixed fuel cost per rental
    fuel_unit = db.Column(db.String(50), default='per_rental')  # per_rental or per_day
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class Rental(db.Model):
    ...
    rental_cost = db.Column(db.Float, nullable=False)  # Daily rental cost only
    fuel_cost = db.Column(db.Float, default=0.0)  # Fixed fuel cost for this rental
    total_cost = db.Column(db.Float, nullable=False)  # rental_cost + fuel_cost
...
def equipment_detail(id):
    ...
    # Use platform default if owner has not set a fuel fixation
    fuel_fixation = equipment.fuel_fixation if equipment.fuel_fixation and equipment.fuel_fixation > 0 else DEFAULT_FUEL_FIXATION
    fuel_unit = equipment.fuel_unit if equipment.fuel_fixation and equipment.fuel_fixation > 0 else DEFAULT_FUEL_UNIT
    return render_template('equipment_detail.html', equipment=equipment, fuel_fixation=fuel_fixation, fuel_unit=fuel_unit)
...
def rental_request(equipment_id):
    ...
    if request.method == 'POST':
        ...
        rental_cost = equipment.price_per_day * duration
        
        # Calculate fuel cost based on effective fixation/unit
        if fuel_unit == 'per_day':
            fuel_cost = fuel_fixation * duration
        else:  # per_rental
            fuel_cost = fuel_fixation
...
def edit_equipment(id):
    ...
    if request.method == 'POST':
        ...
        # Do not accept owner-provided fuel fixation; platform default is used
...
@app.route('/owner/fuel-management')
@login_required
def owner_fuel_management():
    if current_user.role != 'owner':
        return redirect(url_for('farmer_dashboard'))
    
    # Get all equipment with fuel fixation
    equipment_list = Equipment.query.all()
    
    # Calculate fuel revenue for each equipment
    equipment_fuel_stats = []
    for equip in equipment_list:
        completed_rentals = Rental.query.filter_by(equipment_id=equip.id, status='completed').all()
        total_fuel_revenue = sum(rental.fuel_cost for rental in completed_rentals)
        rental_count = len(completed_rentals)
        
        equipment_fuel_stats.append({
            'equipment': equip,
            'total_fuel_revenue': total_fuel_revenue,
            'rental_count': rental_count,
            'avg_fuel_cost': total_fuel_revenue / rental_count if rental_count > 0 else 0
        })
    
    return render_template('owner_dashboard.html', 
                         fuel_management_mode=True,
                         equipment_fuel_stats=equipment_fuel_stats)
...
def owner_analytics():
    ...
    total_rental_revenue = db.session.query(db.func.sum(Rental.rental_cost)).filter_by(status='completed').scalar() or 0
    total_fuel_revenue = db.session.query(db.func.sum(Rental.fuel_cost)).filter_by(status='completed').scalar() or 0
    ...
    # Get monthly fuel revenue
    monthly_fuel_data = db.session.query(
        db.func.strftime('%Y-%m', Rental.created_at),
        db.func.sum(Rental.fuel_cost)
    ).filter_by(status='completed').group_by(db.func.strftime('%Y-%m', Rental.created_at)).all()
